...
struct MCB_s *MCB_head = NULL;
uint64 cur_map = USER_FREE_ADDRESS_START;
...
void malloc_map(uint64 n){
  uint64 temp = ROUNDUP(cur_map, PGSIZE);
  char *new_mem;
  for(uint64 i=temp; i < cur_map+n; i += PGSIZE)
  {
...
void MCB_init() {
  if(first_malloc) {
    uint64 va_top = cur_map;
    malloc_map(sizeof(MCB));
