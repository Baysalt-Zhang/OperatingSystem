...
elf_status elf_load(elf_ctx *ctx) {
  ...
  for (i = 0, off = ctx->ehdr.ph_off; i < ctx->ehdr.ph_num; i++, off += sizeof(ph_addr)) {
...
void load_debug_line(elf_ctx *ctx)
{
  ...
  uint16 sect_num = ctx->ehdr.sh_num;
  uint64 str_offset = ctx->ehdr.sh_off + ctx->ehdr.sh_strndx * sizeof(elf_sect_header);
  elf_fpread(ctx, (void *)&str_sh, sizeof(str_sh), str_offset);

  char temp_str[str_sh.size];
  uint64 str_sect_off = str_sh.offset;
  elf_fpread(ctx, &temp_str, str_sh.size, str_sect_off);

  for(int i=0; i<sect_num; i++) {
    elf_fpread(ctx, (void*)&temp_sh, sizeof(temp_sh), ctx->ehdr.sh_off+i*ctx->ehdr.sh_entsize);
